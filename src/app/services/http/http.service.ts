...
export class HttpService<T extends Resource> {
  ...
  public create(item: T): Observable<T> {
    return this.httpClient.post<T>(`${this.url}/${this.endpoint}`,
      this.serializer.toJson(item), this.options)
      .pipe(
  ...
  public read(id: number): Observable<T> {
    return this.httpClient
      .get(`${this.url}/${this.endpoint}/${id}`, this.options)
      .pipe(
  ...
  public showAll(): Observable<T[]> {
    return this.httpClient
      .get<T[]>(`${this.url}/${this.endpoint}`, this.options)
      .pipe(
        retry(1),
        catchError(this.errorHandl)
      );
  ...
  public delete(id: number) {
    return this.httpClient
      .delete(`${this.url}/${this.endpoint}/${id}`, this.options)
      .pipe(
        retry(1),
        catchError(this.errorHandl)
      );
